...
function <portlet:namespace />doDeleteItems(){
	...
	//alert(_urlDelete)
	if (agree){
		 var form = document.forms['organizationExtMasterForm'];
...
function <portlet:namespace />doSubmitForm(){
    var organizationExt={
    		organizationExtCode:$('#organizationExtCode').val()
    }
   // alert(researchGroup.groupCode)
   if($("#<portlet:namespace/>mode").val()=='add'){
	  ResearchAjax.checkUQOrganizationExt(organizationExt,  function(data) {
	     if(data>0){
	    	 alert(" ข้อมูลซ้ำ  ")
	    	 $('#organizationExtCode').focus()
	    	 return false;
	     }else{
...
<html lang="en">
  ...
  <body>
<form:form  id="organizationExtMasterForm" modelAttribute="organizationExtMasterForm" method="post"  name="organizationExtMasterForm" action="${formAction}" enctype="multipart/form-data">
   <fieldset style="font-family: sans-serif;padding-top:5px"> 
           <input type="hidden" name="command" id="<portlet:namespace />common" value="${organizationExtMasterForm.command}" />
              <input type="hidden" name="mode" id="<portlet:namespace />mode"  value="${organizationExtMasterForm.mode}" />
                <input type="hidden" name="pageNo"  id="<portlet:namespace />pageNo"  value="${organizationExtMasterForm.pageNo}" />
            <input type="hidden" id="<portlet:namespace />pageCount" value="${organizationExtMasterForm.pageCount}" />
  ...
  <strong>Import Success !</strong>
                 ...
                 <tr>
              		<td align="right" width="20%">
                        <span>
                        	<label>รหัสบริษัท/องค์กร:<span style="color: red;font-size: 50;"> </span> 
                        	</label>
                        </span> 
             	 	</td>
              	 	<td width="80%" colspan="3">
                    	<div> 
                    	<form:input path="organizationExtM.organizationExtCode" cssStyle="display:inline;width:50px" cssClass="form-control"  id="organizationExtCode" placeholder=""/>   
                       </div>
              		</td> 
            	</tr> 
            	  <tr>
              		<td align="right" width="20%">
                        <span>
                        	<label>ชื่อบริษัท/องค์กร:<span style="color: red;font-size: 50;"> </span> 
                        	</label>
                        </span> 
             	 	</td>
              	 	<td width="80%" colspan="3">
                    	<div> 
                    	 <form:hidden path="organizationExtM.organizationExtId"  id="organizationExtId"/>
                    	 <form:input path="organizationExtM.organizationExtName" cssStyle="display:inline;width:400px" cssClass="form-control"  id="organizationExtName" placeholder=""/>
                       </div>
              		</td> 
            	</tr>    
              ...
              <td align="center" colspan="4" width="20%">
               ...
               <thead>    
                   ...
                   <th width="30%"><div class="th_class">รหัสบริษัท/องค์กร</div></th>  
                    <th width="47%"><div class="th_class">ชื่อบริษัท/องค์กร</div></th> 
                ...
                <tbody>    
                <c:if test="${not empty organizationExts}"> 
                 <c:forEach items="${organizationExts}" var="organizationExt" varStatus="loop"> 
               		 <tr style="cursor: pointer;">
                 <td style="text-align: left"><form:checkbox path="ids" id="${ns}_x_${organizationExt.organizationExtId}" value="${organizationExt.organizationExtId}"/></td>  
                		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="organizationExtId"><jsp:attribute name="value"><c:out value="${organizationExt.organizationExtId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${organizationExt.organizationExtId}")' style="text-align: left">${organizationExt.organizationExtCode}</td>  
               	 		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="organizationExtId"><jsp:attribute name="value"><c:out value="${organizationExt.organizationExtId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${organizationExt.organizationExtId}")' style="text-align: left">${organizationExt.organizationExtName}</td>  
               	 				<fmt:formatDate type="time" value="${organizationExt.createdDate}"   pattern="dd/MM/yyyy" var="createdDate" /> 
                		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="organizationExtId"><jsp:attribute name="value"><c:out value="${organizationExt.organizationExtId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${organizationExt.organizationExtId}")' style="text-align: left">${createdDate} </td>  
                		 <fmt:formatDate type="time" value="${organizationExt.updatedDate}"   pattern="dd/MM/yyyy" var="updatedDate" /> 
                		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="organizationExtId"><jsp:attribute name="value"><c:out value="${organizationExt.organizationExtId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${organizationExt.organizationExtId}")' style="text-align: left">${updatedDate} </td> 
                		<td>
                		<div class="btn-group">
                			<a class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#">
  							...
  							<ul class="dropdown-menu">
   								<li><a onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="organizationExtId"><jsp:attribute name="value"><c:out value="${organizationExt.organizationExtId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${organizationExt.organizationExtId}")' ><i class="icon-edit"></i> Edit</a></li>
    							<li><a onclick='<portlet:namespace />displayElementEdit("copy","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="organizationExtId"><jsp:attribute name="value"><c:out value="${organizationExt.organizationExtId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${organizationExt.organizationExtId}")'  ><i class="icon-plus-sign"></i> Copy</a></li>
    							<li><a onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="organizationExtId"><jsp:attribute name="value"><c:out value="${organizationExt.organizationExtId}"/></jsp:attribute></portlet:param></portlet:actionURL>")' ><i class="icon-trash"></i> Delete</a></li>
  							</ul>
						</div>
<%--
                  				<button onclick='displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="organizationExtId"><jsp:attribute name="value"><c:out value="${organizationExt.organizationExtId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${organizationExt.organizationExtId}")'  class="btn btn-small" type="button">คัดลอก</button>
 						   	 <button onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="organizationExtId"><jsp:attribute name="value"><c:out value="${organizationExt.organizationExtId}"/></jsp:attribute></portlet:param></portlet:actionURL>")'   class="btn btn-danger btn-small" type="button">ลบ</button>
 	--%>
 				<%--
                  			<i onclick='displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="organizationExtId"><jsp:attribute name="value"><c:out value="${organizationExt.organizationExtId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${organizationExt.organizationExtId}")' class="icon-edit"></i>
                			<i  onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="organizationExtId"><jsp:attribute name="value"><c:out value="${organizationExt.organizationExtId}"/></jsp:attribute></portlet:param></portlet:actionURL>")' class="icon-trash"></i></td>
                			 --%> 
                			 </td>
              		</tr>
              </c:forEach>
           ...
           <table border="0" width="100%" style="font-size: 13px">
                ...
                <span class="pagination">
 				 <ul>
    			 <c:if test="${organizationExtMasterForm.pageNo!=1}">
    				<li><a style="cursor: pointer;" onclick='<portlet:namespace/>goPrev()'>Prev</a></li>
    			</c:if>
    			 <c:if test="${not empty organizationExtMasterForm.pageCount}">
                     <c:forEach begin="${organizationExtMasterForm.pageStart}" end="${organizationExtMasterForm.pageEnd}"  var="loop">
                 	<c:if test="${organizationExtMasterForm.pageNo==(loop)}">
                ...
                <c:if test="${organizationExtMasterForm.pageEnd<organizationExtMasterForm.pageCount}">
                 		<li><a class="active" style="cursor: pointer;" onclick='<portlet:namespace/>goToPage("${organizationExtMasterForm.pageEnd}")'>...</a></li>
                 		<li><a class="active" style="cursor: pointer;" onclick='<portlet:namespace/>goToPage("${organizationExtMasterForm.pageCount}")'>${organizationExtMasterForm.pageCount}</a></li>
                 </c:if>
                 </c:if>
                 <c:if test="${organizationExtMasterForm.pageNo!=organizationExtMasterForm.pageCount}">
...
<script type="text/javascript" src="<c:url value='/resources/js/jquery.iframe-transport.js'/>"></script>
  ...
  <script type="text/javascript" src="<c:url value='/resources/js/jquery.fileupload.js'/>"></script>   
       ...
       function <portlet:namespace />displayElementEdit(mode,url,id){
       	   $("#organizationExtCode").val('');
       	   $("#organizationExtName").val('');
  	   ...
  	   if(mode=='edit'){
  		   $("#<portlet:namespace/>export_import_element").show(); 
       	      	    $.ajax({
       	      	  	url: url , 
       	      	     	 data: { organizationExtId: id }, 
       	      	         success: function(data){
       	      	        $('#organizationExtCode').prop('readonly',true);
       	      	        	 $("#organizationExtId").val(id)
       	      	        	 $("#organizationExtCode").val(data.organizationExtCode)
       		   				$("#organizationExtName").val(data.organizationExtName);
      		 ...
      		 $("#<portlet:namespace/>mode").val('add');
  	      	    $.ajax({
  	      	  	url: url , 
  	      	     	 data: { organizationExtId: id }, 
  	      	         success: function(data){
  	      	        $('#organizationExtCode').prop('readonly',false);
  	      	        	// $("#organizationExtId").val(id)
  	      	        	 //$("#organizationExtCode").val(data.organizationExtCode)
  		   				$("#organizationExtName").val(data.organizationExtName);
