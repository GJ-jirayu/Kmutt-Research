...
function <portlet:namespace />doDeleteItems(){
	...
	//alert(_urlDelete)
	if (agree){
		 var form = document.forms['journalPapersTypeMasterForm'];
...
function <portlet:namespace />doSubmitForm(){
    var journalPapersType={
    		jptCode:$('#jptCode').val()
    }
   // alert(researchGroup.groupCode)
   if($("#<portlet:namespace/>mode").val()=='add'){
	  ResearchAjax.checkUQJournalPapersType(journalPapersType,  function(data) {
	     if(data>0){
	    	 alert(" ข้อมูลซ้ำ  ")
	    	 $('#jptCode').focus()
	    	 return false;
...
<html lang="en">
  <head>
   ...
   <meta http-equiv="Pragma" content="no-cache">
    ...
    <title>JournalPapers Type</title>
  ...
  <body>
<form:form  id="journalPapersTypeMasterForm" modelAttribute="journalPapersTypeMasterForm" method="post"  name="journalPapersTypeMasterForm" action="${formAction}" enctype="multipart/form-data">
   <fieldset style="font-family: sans-serif;padding-top:5px"> 
           <input type="hidden" name="command" id="<portlet:namespace />common" value="${journalPapersTypeMasterForm.command}" />
              <input type="hidden" name="mode" id="<portlet:namespace />mode"  value="${journalPapersTypeMasterForm.mode}" />
                <input type="hidden" name="pageNo"  id="<portlet:namespace />pageNo"  value="${journalPapersTypeMasterForm.pageNo}" />
            <input type="hidden" id="<portlet:namespace />pageCount" value="${journalPapersTypeMasterForm.pageCount}" />
  ...
  <strong>Import Success !</strong>
                 ...
                 <tr>
              		<td align="right" width="20%">
                        <span>
                        	<label>รหัสประเภทบทความ:<span style="color: red;font-size: 50;"> </span> 
                        	</label>
                        </span> 
             	 	</td>
              	 	<td width="80%" colspan="3">
              	 		<div>
                    		<form:input path="journalPapersTypeM.jptCode" cssStyle="display:inline;width:50px" cssClass="form-control"  id="jptCode" placeholder=""/>
                       </div>
              		</td> 
            	</tr> 
            	  <tr>
              		<td align="right" width="20%">
                        <span>
                        	<label>ชื่อประเภทบทความ:<span style="color: red;font-size: 50;"> </span> 
                        	</label>
                        </span> 
             	 	</td>
              	 	<td width="80%" colspan="3">
                    	<div>
                    	 <form:hidden path="journalPapersTypeM.journalPapersTypeId"  id="journalPapersTypeId"/>
                    	  <form:input path="journalPapersTypeM.jptName" cssStyle="display:inline;width:400px" cssClass="form-control"  id="jptName" placeholder=""/>
                       </div>
              		</td> 
            	</tr>  
              <tr>
              <td align="center" colspan="4" width="20%">
               ...
               <thead>    
                   ...
                   <th width="15%"><div class="th_class">รหัสประเภทบทความ</div></th>  
                    <th width="62%"><div class="th_class">ชื่อประเภทบทความ</div></th>
                ...
                <tbody>    
               <c:if test="${not empty journalPapersTypes}"> 
                 <c:forEach items="${journalPapersTypes}" var="journalPapersType" varStatus="loop"> 
               		 <tr style="cursor: pointer;">
                		 <td style="text-align: left"><form:checkbox path="ids" id="${ns}_x_${journalPapersType.journalPapersTypeId}" value="${journalPapersType.journalPapersTypeId}"/></td>  
                		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="journalPapersTypeId"><jsp:attribute name="value"><c:out value="${journalPapersType.journalPapersTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${journalPapersType.journalPapersTypeId}")' style="text-align: left">${journalPapersType.jptCode}</td>  
               	 		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="journalPapersTypeId"><jsp:attribute name="value"><c:out value="${journalPapersType.journalPapersTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${journalPapersType.journalPapersTypeId}")' style="text-align: left">${journalPapersType.jptName}</td> 
               	 		<fmt:formatDate type="time" value="${journalPapersType.createdDate}"   pattern="dd/MM/yyyy" var="createdDate" /> 
                		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="journalPapersTypeId"><jsp:attribute name="value"><c:out value="${journalPapersType.journalPapersTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${journalPapersType.journalPapersTypeId}")' style="text-align: left">${createdDate} </td>  
                		 <fmt:formatDate type="time" value="${journalPapersType.updatedDate}"   pattern="dd/MM/yyyy" var="updatedDate" /> 
                		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="journalPapersTypeId"><jsp:attribute name="value"><c:out value="${journalPapersType.journalPapersTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${journalPapersType.journalPapersTypeId}")' style="text-align: left">${updatedDate} </td>  
                		<td>
                		<div class="btn-group">
                			<a class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#">
  							...
  							<ul class="dropdown-menu">
   								<li><a onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="journalPapersTypeId"><jsp:attribute name="value"><c:out value="${journalPapersType.journalPapersTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${journalPapersType.journalPapersTypeId}")'><i class="icon-edit"></i> Edit</a></li>
    							<li><a onclick='<portlet:namespace />displayElementEdit("copy","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="journalPapersTypeId"><jsp:attribute name="value"><c:out value="${journalPapersType.journalPapersTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${journalPapersType.journalPapersTypeId}")' ><i class="icon-plus-sign"></i> Copy</a></li>
    							<li><a onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="journalPapersTypeId"><jsp:attribute name="value"><c:out value="${journalPapersType.journalPapersTypeId}"/></jsp:attribute></portlet:param></portlet:actionURL>")'  ><i class="icon-trash"></i> Delete</a></li>
  							</ul>
						</div>
<%--
                  			 <button onclick='displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="journalPapersTypeId"><jsp:attribute name="value"><c:out value="${journalPapersType.journalPapersTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${journalPapersType.journalPapersTypeId}")' class="btn btn-small" type="button">คัดลอก</button>
 						   	 <button  onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="journalPapersTypeId"><jsp:attribute name="value"><c:out value="${journalPapersType.journalPapersTypeId}"/></jsp:attribute></portlet:param></portlet:actionURL>")' class="btn btn-danger btn-small" type="button">ลบ</button>
 	--%>
 			<%--
                  			<i onclick='displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="journalPapersTypeId"><jsp:attribute name="value"><c:out value="${journalPapersType.journalPapersTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${journalPapersType.journalPapersTypeId}")' class="icon-edit"></i>
                			<i  onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="journalPapersTypeId"><jsp:attribute name="value"><c:out value="${journalPapersType.journalPapersTypeId}"/></jsp:attribute></portlet:param></portlet:actionURL>")' class="icon-trash"></i></td>
                			--%>
                			</td> 
              		</tr>
              </c:forEach>
           ...
           <table border="0" width="100%" style="font-size: 13px">
                ...
                <span class="pagination">
 				 <ul>
    			 <c:if test="${journalPapersTypeMasterForm.pageNo!=1}">
    				<li><a style="cursor: pointer;" onclick='<portlet:namespace/>goPrev()'>Prev</a></li>
    			</c:if>
    			 <c:if test="${not empty journalPapersTypeMasterForm.pageCount}">
                     <c:forEach begin="${journalPapersTypeMasterForm.pageStart}" end="${journalPapersTypeMasterForm.pageEnd}"  var="loop">
                 	<c:if test="${journalPapersTypeMasterForm.pageNo==(loop)}">
                ...
                <c:if test="${journalPapersTypeMasterForm.pageEnd<journalPapersTypeMasterForm.pageCount}">
                 		<li><a class="active" style="cursor: pointer;" onclick='<portlet:namespace/>goToPage("${journalPapersTypeMasterForm.pageEnd}")'>...</a></li>
                 		<li><a class="active" style="cursor: pointer;" onclick='<portlet:namespace/>goToPage("${journalPapersTypeMasterForm.pageCount}")'>${journalPapersTypeMasterForm.pageCount}</a></li>
                 </c:if>
                 </c:if>
                 <c:if test="${journalPapersTypeMasterForm.pageNo!=journalPapersTypeMasterForm.pageCount}">
       ...
       function <portlet:namespace />displayElementEdit(mode,url,id){
  	   ...
  	   if(mode=='edit'){
  		   $("#<portlet:namespace/>export_import_element").show(); 
       	      	    $.ajax({
       	      	  	url: url , 
       	      	     	 data: { journalPapersTypeId: id }, 
       	      	         success: function(data){
       	      	        $('#jptCode').prop('readonly',true);
       	      	        	 $("#journalPapersTypeId").val(id)
       	      	        	 $("#jptCode").val(data.jptCode)
       		   				$("#jptName").val(data.jptName);
