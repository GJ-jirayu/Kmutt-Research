...
function <portlet:namespace />doDeleteItems(){
	...
	//alert(_urlDelete)
	if (agree){
		 var form = document.forms['countryMasterForm'];
...
function <portlet:namespace />doSubmitForm(){
   ...
   if($("#<portlet:namespace/>mode").val()=='add'){
	  ResearchAjax.checkUQCountry(country,  function(data) {
	     if(data>0){
	    	 alert(" ข้อมูลซ้ำ  ")
	    	 $('#countryCode').focus()
	    	 return false;
...
<html lang="en">
  ...
  <body>
<form:form  id="countryMasterForm" modelAttribute="countryMasterForm" method="post"  name="countryMasterForm" action="${formAction}" enctype="multipart/form-data">
   <fieldset style="font-family: sans-serif;padding-top:5px"> 
             <input type="hidden" name="command" id="<portlet:namespace />common" value="${countryMasterForm.command}" />
              <input type="hidden" name="mode" id="<portlet:namespace />mode"  value="${countryMasterForm.mode}" />
                <input type="hidden" name="pageNo"  id="<portlet:namespace />pageNo"  value="${countryMasterForm.pageNo}" />
            <input type="hidden" id="<portlet:namespace />pageCount" value="${countryMasterForm.pageCount}" />
         ...
         <input type="hidden" id="<portlet:namespace />_export_csv"/> 
                 ...
                 <tr>
              		<td align="right" width="20%">
                        <span>
                        	<label>รหัสประเทศ:<span style="color: red;font-size: 50;"> </span> 
                        	</label>
                        </span> 
             	 	</td>
              	 	<td width="80%" colspan="3">
                    	<div> 
                    	<form:input path="countryM.countryCode" cssStyle="display:inline;width:50px" cssClass="form-control"  id="countryCode" placeholder=""/>   
                       </div>
              		</td> 
            	</tr> 
            	  <tr>
              		<td align="right" width="20%">
                        <span>
                        	<label>ชื่อประเทศ(T):<span style="color: red;font-size: 50;"> </span> 
                        	</label>
                        </span> 
             	 	</td>
              	 	<td width="80%" colspan="3">
                    	<div> 
                    	<form:hidden path="countryM.countryId"  id="countryId"/>
                    	 <form:input path="countryM.countryNameTh" cssStyle="display:inline;width:400px" cssClass="form-control"  id="countryNameTh" placeholder=""/>
                       </div>
              		</td> 
            	</tr> 
            	  <tr>
              		<td align="right" width="20%">
                        <span>
                        	<label>ชื่อประเทศ(E):<span style="color: red;font-size: 50;"> </span> 
                        	</label>
                        </span> 
             	 	</td>
              	 	<td width="80%" colspan="3">
                    	<div> 
                    	 <form:input path="countryM.countryNameEng" cssStyle="display:inline;width:400px" cssClass="form-control"  id="countryNameEng" placeholder=""/>   
                       </div>
              		</td> 
            	</tr> 
              <tr>
              <td align="center" colspan="4" width="20%">
               ...
               <thead>    
                   ...
                   <th width="9%"><div class="th_class">รหัสประเทศ</div></th>  
                    <th width="34%"><div class="th_class">ชื่อประเทศ(T)</div></th>  
                    <th width="34%"><div class="th_class">ชื่อประเทศ(E)</div></th>   
                    <th width="5%"><div class="th_class">Created</div></th>  
                      <th width="5%"><div class="th_class">Updated</div></th>
					   <th width="5%"><div class="th_class">Updated By</div></th>
                <th width="11%"><div class="th_class"></div></th>  
                   </tr> 
                </thead> 
                <tbody>   
                 <c:if test="${not empty countrys}"> 
                 <c:forEach items="${countrys}" var="country" varStatus="loop"> 
               		 <tr style="cursor: pointer;">
                 	<td><form:checkbox path="ids" id="${ns}_x_${country.countryId}" value="${country.countryId}"/></td>  
                		<td style="text-align: left" onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${country.countryId}")' >${country.countryCode}</td>  
               	 		<td style="text-align: left" onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${country.countryId}")' >${country.countryNameTh}</td>  
               	 		<td style="text-align: left" onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${country.countryId}")' >${country.countryNameEng}</td> 
               	 			<fmt:formatDate type="time" value="${country.createdDate}"   pattern="dd/MM/yyyy" var="createdDate" /> 
                		<td style="text-align: left" onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${country.countryId}")' >${createdDate} </td>  
                		 <fmt:formatDate type="time" value="${country.updatedDate}"   pattern="dd/MM/yyyy" var="updatedDate" /> 
                		<td style="text-align: left" onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${country.countryId}")' >${updatedDate} </td>
						 <td style="text-align: left" onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${country.countryId}")' >${country.updatedBy} </td>
						 <td>
                		<div class="btn-group">
                			<a class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#">
  							...
  							<ul class="dropdown-menu">
   								<li><a onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${country.countryId}")'><i class="icon-edit"></i> Edit</a></li>
    							<li><a onclick='<portlet:namespace />displayElementEdit("copy","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${country.countryId}")'  ><i class="icon-plus-sign"></i> Copy</a></li>
    							<li><a onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:actionURL>")' ><i class="icon-trash"></i> Delete</a></li>
  							</ul>
						</div>	
<%--
                  				<button onclick='displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${country.countryId}")' class="btn btn-small" type="button">คัดลอก</button>
 						   	 <button onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:actionURL>")'  class="btn btn-danger btn-small" type="button">ลบ</button>
 	--%>
 			 			 <%--
                  			<i onclick='displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${country.countryId}")' class="icon-edit"></i>
                			<i  onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="countryId"><jsp:attribute name="value"><c:out value="${country.countryId}"/></jsp:attribute></portlet:param></portlet:actionURL>")' class="icon-trash"></i></td>
                			 --%>
                			 </td> 
              		</tr>
              </c:forEach>
           ...
           <table border="0" width="100%" style="font-size: 13px">
                ...
                <span class="pagination">
 				 <ul>
    			 <c:if test="${countryMasterForm.pageNo!=1}">
    				<li><a style="cursor: pointer;" onclick='<portlet:namespace/>goPrev()'>Prev</a></li>
    			</c:if>
    			 <c:if test="${not empty countryMasterForm.pageCount}">
                     <c:forEach begin="${countryMasterForm.pageStart}" end="${countryMasterForm.pageEnd}"  var="loop">
                 	<c:if test="${countryMasterForm.pageNo==(loop)}">
                ...
                <c:if test="${countryMasterForm.pageEnd<countryMasterForm.pageCount}">
                 		<li><a class="active" style="cursor: pointer;" onclick='<portlet:namespace/>goToPage("${countryMasterForm.pageEnd}")'>...</a></li>
                 		<li><a class="active" style="cursor: pointer;" onclick='<portlet:namespace/>goToPage("${countryMasterForm.pageCount}")'>${countryMasterForm.pageCount}</a></li>
                 </c:if>
                 </c:if>
                 <c:if test="${countryMasterForm.pageNo!=countryMasterForm.pageCount}">
        	      	    ...
        	      	    $.ajax({
        	      	  	url: url , 
        	      	         ...
        	      	         success: function(data){
        		   				...
        		   				$("#countryNameTh").val(data.countryNameTh);
        	      	        	$("#countryNameEng").val(data.countryNameEng);
