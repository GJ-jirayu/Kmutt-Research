...
function <portlet:namespace />doDeleteItems(){
	...
	//alert(_urlDelete)
	if (agree){
		 var form = document.forms['publishedTypeMasterForm'];
...
function <portlet:namespace />doSubmitForm(){
   ...
   if($("#<portlet:namespace/>mode").val()=='add'){
	  ResearchAjax.checkUQPublishType(publishType,  function(data) {
	     if(data>0){
	    	 alert(" ข้อมูลซ้ำ  ")
	    	 $('#ptCode').focus()
	    	 return false;
...
<html lang="en">
  ...
  <body>
  <form:form  id="publishedTypeMasterForm" modelAttribute="publishedTypeMasterForm" method="post"  name="publishedTypeMasterForm" action="${formAction}" enctype="multipart/form-data">
   
   <fieldset style="font-family: sans-serif;padding-top:5px"> 
          <input type="hidden" name="command" id="<portlet:namespace />common" value="${publishedTypeMasterForm.command}" />
              <input type="hidden" name="mode" id="<portlet:namespace />mode"  value="${publishedTypeMasterForm.mode}" />
                <input type="hidden" name="pageNo"  id="<portlet:namespace />pageNo"  value="${publishedTypeMasterForm.pageNo}" />
            <input type="hidden" id="<portlet:namespace />pageCount" value="${publishedTypeMasterForm.pageCount}" />
  ...
  <strong>Import Success !</strong>
                 ...
                 <tr>
              		<td align="right" width="20%">
                        <span>
                        	<label>รหัสชนิดแหล่งตีพิมพ์:<span style="color: red;font-size: 50;"> </span> 
                        	</label>
                        </span> 
             	 	</td>
              	 	<td width="80%" colspan="3">
              	 		<div>
                    	<form:input path="publishTypeM.ptCode" cssStyle="display:inline;width:50px" cssClass="form-control"  id="ptCode" placeholder=""/>
                       </div>
              		</td> 
            	</tr> 
            	  <tr>
              		<td align="right" width="20%">
                        <span>
                        	<label>ชื่อชนิดแหล่งตีพิมพ์:<span style="color: red;font-size: 50;"> </span> 
                        	</label>
                        </span> 
             	 	</td>
              	 	<td width="80%" colspan="3">
                    	<div> 
                    	 <form:hidden path="publishTypeM.publishTypeId"  id="publishTypeId"/>
                    	  <form:input path="publishTypeM.ptName" cssStyle="display:inline;width:400px" cssClass="form-control"  id="ptName" placeholder=""/>
                       </div>
              		</td> 
            	</tr>    
              <tr>
           ...
           <button type="button" onclick="<portlet:namespace />doSubmitForm()" class="btn btn-primary">Submit</button>
               ...
               <thead>    
                   ...
                   <th width="29%"><div class="th_class">รหัสชนิดแหล่งตีพิมพ์</div></th>  
                    <th width="48%"><div class="th_class">ชื่อชนิดแหล่งตีพิมพ์</div></th> 
                ...
                <tbody>    
                <c:if test="${not empty publishedTypes}"> 
                 <c:forEach items="${publishedTypes}" var="publishedType" varStatus="loop"> 
               		 <tr style="cursor: pointer;">
                 <td style="text-align: left"><form:checkbox path="ids" id="${ns}_x_${publishedType.publishTypeId}" value="${publishedType.publishTypeId}"/></td>  
                		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="publishedTypeId"><jsp:attribute name="value"><c:out value="${publishedType.publishTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${publishedType.publishTypeId}")' style="text-align: left">${publishedType.ptCode}</td>  
               	 		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="publishedTypeId"><jsp:attribute name="value"><c:out value="${publishedType.publishTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${publishedType.publishTypeId}")' style="text-align: left">${publishedType.ptName}</td>  
               	 				<fmt:formatDate type="time" value="${publishedType.createdDate}"   pattern="dd/MM/yyyy" var="createdDate" /> 
                		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="publishedTypeId"><jsp:attribute name="value"><c:out value="${publishedType.publishTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${publishedType.publishTypeId}")' style="text-align: left">${createdDate} </td>  
                		 <fmt:formatDate type="time" value="${publishedType.updatedDate}"   pattern="dd/MM/yyyy" var="updatedDate" /> 
                		...
                		<div class="btn-group">
                			<a class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#">
  							...
  							<ul class="dropdown-menu">
    							...
    							<li><a onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="publishedTypeId"><jsp:attribute name="value"><c:out value="${publishedType.publishTypeId}"/></jsp:attribute></portlet:param></portlet:actionURL>")'  ><i class="icon-trash"></i> Delete</a></li>
  							</ul>
						</div>
<%--
                  				<button onclick='displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="publishedTypeId"><jsp:attribute name="value"><c:out value="${publishedType.publishTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${publishedType.publishTypeId}")'  class="btn btn-small" type="button">คัดลอก</button>
 						   	 <button  onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="publishedTypeId"><jsp:attribute name="value"><c:out value="${publishedType.publishTypeId}"/></jsp:attribute></portlet:param></portlet:actionURL>")'  class="btn btn-danger btn-small" type="button">ลบ</button>
 	 --%>
 					<%--
                  			<i onclick='displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="publishedTypeId"><jsp:attribute name="value"><c:out value="${publishedType.publishTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${publishedType.publishTypeId}")' class="icon-edit"></i>
                			<i  onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="publishedTypeId"><jsp:attribute name="value"><c:out value="${publishedType.publishTypeId}"/></jsp:attribute></portlet:param></portlet:actionURL>")' class="icon-trash"></i></td>
                			 --%> 
                			 </td>
              		</tr>
              </c:forEach>
           ...
           <table border="0" width="100%" style="font-size: 13px">
                ...
                <span class="pagination">
 				 <ul>
    			 <c:if test="${publishedTypeMasterForm.pageNo!=1}">
    				<li><a style="cursor: pointer;" onclick='<portlet:namespace/>goPrev()'>Prev</a></li>
    			</c:if>
    			 <c:if test="${not empty publishedTypeMasterForm.pageCount}">
                     <c:forEach begin="${publishedTypeMasterForm.pageStart}" end="${publishedTypeMasterForm.pageEnd}"  var="loop">
                 	<c:if test="${publishedTypeMasterForm.pageNo==(loop)}">
                ...
                <c:if test="${publishedTypeMasterForm.pageEnd<publishedTypeMasterForm.pageCount}">
                 		<li><a class="active" style="cursor: pointer;" onclick='<portlet:namespace/>goToPage("${publishedTypeMasterForm.pageEnd}")'>...</a></li>
                 		<li><a class="active" style="cursor: pointer;" onclick='<portlet:namespace/>goToPage("${publishedTypeMasterForm.pageCount}")'>${publishedTypeMasterForm.pageCount}</a></li>
                 </c:if>
                 </c:if>
                 <c:if test="${publishedTypeMasterForm.pageNo!=publishedTypeMasterForm.pageCount}">
  ...
  <script type="text/javascript" src="<c:url value='/resources/js/jquery.fileupload.js'/>"></script>   
	   ...
	   if(mode=='edit'){
		   $("#<portlet:namespace/>export_import_element").show(); 
   	      	    $.ajax({
   	      	  	url: url , 
   	      	     	 data: { publishedTypeId: id }, 
   	      	         success: function(data){
   	      	        $('#ptCode').prop('readonly',true);
   	      	        	 $("#publishTypeId").val(id)
   	      	        	 $("#ptCode").val(data.ptCode)
