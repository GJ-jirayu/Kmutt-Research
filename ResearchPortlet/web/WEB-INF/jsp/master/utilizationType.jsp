...
function <portlet:namespace />doDeleteItems(){
	...
	//alert(_urlDelete)
	if (agree){
		 var form = document.forms['utilizationTypeMasterForm'];
...
function <portlet:namespace />doSubmitForm(){
    var utilizationType={
    		utilizationCode:$('#utilizationCode').val()
    }
   // alert(researchGroup.groupCode)
   if($("#<portlet:namespace/>mode").val()=='add'){
	  ResearchAjax.checkUQUtilizationType(utilizationType,  function(data) {
	     if(data>0){
	    	 alert(" ข้อมูลซ้ำ  ")
	    	 $('#utilizationCode').focus()
	    	 return false;
	     }else{
...
<html lang="en">
  ...
  <body>
<form:form  id="utilizationTypeMasterForm" modelAttribute="utilizationTypeMasterForm" method="post"  name="utilizationTypeMasterForm" action="${formAction}" enctype="multipart/form-data">
   <fieldset style="font-family: sans-serif;padding-top:5px"> 
           <input type="hidden" name="command" id="<portlet:namespace />common" value="${utilizationTypeMasterForm.command}" />
              <input type="hidden" name="mode" id="<portlet:namespace />mode"  value="${utilizationTypeMasterForm.mode}" />
                <input type="hidden" name="pageNo"  id="<portlet:namespace />pageNo"  value="${utilizationTypeMasterForm.pageNo}" />
            <input type="hidden" id="<portlet:namespace />pageCount" value="${utilizationTypeMasterForm.pageCount}" />
  ...
  <strong>Import Success !</strong>
                 ...
                 <tr>
              		<td align="right" width="20%">
                        <span>
                        	<label>รหัสด้านการนำไปใช้ประโยชน์:<span style="color: red;font-size: 50;"> </span> 
                        	</label>
                        </span> 
             	 	</td>
              	 	<td width="80%" colspan="3">
                    	<div> 
                    	<form:input path="utilizationTypeM.utilizationCode" cssStyle="display:inline;width:50px" cssClass="form-control"  id="utilizationCode" placeholder=""/>   
                       </div>
              		</td> 
            	</tr> 
            	  <tr>
              		<td align="right" width="20%">
              	 	...
              	 	<td width="80%" colspan="3">
                    	<div>
                    	 <form:hidden path="utilizationTypeM.utilizationTypeId"  id="utilizationTypeId"/>
                    	  ...
                    	  <form:input path="utilizationTypeM.utilizationName" cssStyle="display:inline;width:400px" cssClass="form-control"  id="utilizationName" placeholder=""/>   
                       </div>
              		</td> 
            	</tr>    
              <tr>
              ...
              <button type="button" onclick="<portlet:namespace />doSubmitForm()" class="btn btn-primary">Submit</button>
                ...
                <button  onclick="<portlet:namespace />displayElementEdit('add')" type="button" class="btn btn-primary">Add</button> 
               ...
               <thead>    
                   ...
                   <th width="25%"><div class="th_class">รหัสด้านการนำไปใช้ประโยชน์</div></th>  
                    <th width="52%"><div class="th_class">ด้านการนำไปใช้ประโยชน์</div></th> 
                ...
                <tbody>    
                 <c:if test="${not empty utilizationTypes}"> 
                 <c:forEach items="${utilizationTypes}" var="utilizationType" varStatus="loop"> 
               		 <tr style="cursor: pointer;">
                 <td style="text-align: left"><form:checkbox path="ids" id="${ns}_x_${utilizationType.utilizationTypeId}" value="${utilizationType.utilizationTypeId}"/></td>  
                		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="utilizationTypeId"><jsp:attribute name="value"><c:out value="${utilizationType.utilizationTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${utilizationType.utilizationTypeId}")' style="text-align: left">${utilizationType.utilizationCode}</td>  
               	 		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="utilizationTypeId"><jsp:attribute name="value"><c:out value="${utilizationType.utilizationTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${utilizationType.utilizationTypeId}")' style="text-align: left">${utilizationType.utilizationName}</td>  
               	 			<fmt:formatDate type="time" value="${utilizationType.createdDate}"   pattern="dd/MM/yyyy" var="createdDate" /> 
                		<td onclick='<portlet:namespace />displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="utilizationTypeId"><jsp:attribute name="value"><c:out value="${utilizationType.utilizationTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${utilizationType.utilizationTypeId}")' style="text-align: left">${createdDate} </td>  
                		 <fmt:formatDate type="time" value="${utilizationType.updatedDate}"   pattern="dd/MM/yyyy" var="updatedDate" /> 
                		...
                		<div class="btn-group">
                			<a class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#">
  							...
  							<ul class="dropdown-menu">
    							...
    							<li><a onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="utilizationTypeId"><jsp:attribute name="value"><c:out value="${utilizationType.utilizationTypeId}"/></jsp:attribute></portlet:param></portlet:actionURL>")' ><i class="icon-trash"></i> Delete</a></li>
  							</ul>
						</div>
      <%--          		
                  			<button onclick='displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="utilizationTypeId"><jsp:attribute name="value"><c:out value="${utilizationType.utilizationTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${utilizationType.utilizationTypeId}")' class="btn btn-small" type="button">คัดลอก</button>
 						   	 <button onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="utilizationTypeId"><jsp:attribute name="value"><c:out value="${utilizationType.utilizationTypeId}"/></jsp:attribute></portlet:param></portlet:actionURL>")'  class="btn btn-danger btn-small" type="button">ลบ</button>
 			--%>
 						<%--
                  			<i onclick='displayElementEdit("edit","<portlet:resourceURL  id="research_group_resource_get_byid"><portlet:param name="utilizationTypeId"><jsp:attribute name="value"><c:out value="${utilizationType.utilizationTypeId}"/></jsp:attribute></portlet:param></portlet:resourceURL>","${utilizationType.utilizationTypeId}")' class="icon-edit"></i>
                			<i  onclick='return <portlet:namespace />confirmDelete("<portlet:actionURL><portlet:param name="action" value="delete"/><portlet:param name="utilizationTypeId"><jsp:attribute name="value"><c:out value="${utilizationType.utilizationTypeId}"/></jsp:attribute></portlet:param></portlet:actionURL>")' class="icon-trash"></i></td>
                			--%> 
                			</td>
              		</tr>
              </c:forEach>
           ...
           <table border="0" width="100%" style="font-size: 13px">
                ...
                <span class="pagination">
 				 <ul>
    			 <c:if test="${utilizationTypeMasterForm.pageNo!=1}">
    				<li><a style="cursor: pointer;" onclick='<portlet:namespace/>goPrev()'>Prev</a></li>
    			</c:if>
    			 <c:if test="${not empty utilizationTypeMasterForm.pageCount}">
                     <c:forEach begin="${utilizationTypeMasterForm.pageStart}" end="${utilizationTypeMasterForm.pageEnd}"  var="loop">
                 	<c:if test="${utilizationTypeMasterForm.pageNo==(loop)}">
                ...
                <c:if test="${utilizationTypeMasterForm.pageEnd<utilizationTypeMasterForm.pageCount}">
                 		<li><a class="active" style="cursor: pointer;" onclick='<portlet:namespace/>goToPage("${utilizationTypeMasterForm.pageEnd}")'>...</a></li>
                 		<li><a class="active" style="cursor: pointer;" onclick='<portlet:namespace/>goToPage("${utilizationTypeMasterForm.pageCount}")'>${utilizationTypeMasterForm.pageCount}</a></li>
                 </c:if>
                 </c:if>
                 <c:if test="${utilizationTypeMasterForm.pageNo!=utilizationTypeMasterForm.pageCount}">
       	 ...
       	 $("#<portlet:namespace/>export_import_element").hide(); 
       	   if(mode=='edit'){
       		 $("#<portlet:namespace/>export_import_element").show(); 
       	      	    $.ajax({
       	      	  		 url: url , 
       	      	     //	 data: { utilizationTypeId: id }, 
       	      	         success: function(data){
       	      	        	 //alert(data)
       	      	        	 $('#utilizationCode').prop('readonly',true);
       	      	        	 $("#utilizationTypeId").val(id)
       	      	        	 $("#utilizationCode").val(data.utilizationCode)
       		   				$("#utilizationName").val(data.utilizationName);
         		...
         		$("#<portlet:namespace/>mode").val('add');
    	      	    $.ajax({
    	      	     ...
    	      	     //	 data: { utilizationTypeId: id }, 
    	      	         success: function(data){
    	      	           $("#utilizationCode").prop("readonly",false)
    	      	        	 //alert(data)
    	      	        	// $('#utilizationCode').prop('readonly',true);
    	      	        	 //$("#utilizationTypeId").val(id)
    	      	        	// $("#utilizationCode").val(data.utilizationCode)
